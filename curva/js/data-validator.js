...
const DataValidatorCurva = {
  ...
  async loadExistingRecords(opsToCheck) {
    try {
      ...
      const opsResponse = await fetch(`${CurvaConfig.FUNCTIONS_URL}/verify-ops`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ ops: opsToCheck })
      });
  ...
  async loadBarcodes(itemsToCheck) {
    try {
      const barcodesResponse = await fetch(`${CurvaConfig.FUNCTIONS_URL}/get-barcodes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
