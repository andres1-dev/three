...
/* ── Deslogear Todos los Usuarios ── */
async function logoutAllUsers() {
    const result = await Swal.fire({
        title: '¿Deslogear todos los usuarios?',
        text: 'Esta acción cerrará todas las sesiones activas en el sistema. Todos los usuarios deberán iniciar sesión nuevamente.',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#ef4444',
        cancelButtonColor: '#64748b',
        confirmButtonText: 'Sí, deslogear todos',
    ...
    try {
        Swal.fire({
            title: 'Procesando...',
            text: 'Deslogeando todos los usuarios activos...',
        ...
        try {
            ...
            console.error('[LOGOUT_ALL] Error obteniendo token:', e);
        }

        if (!sessionToken) {
            throw new Error('No se encontró token de sesión válido');
        }

        // Llamar a la Edge Function para deslogear todos los usuarios
        const response = await fetch(`${CONFIG.FUNCTIONS_URL}/operations`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${sessionToken}`,
                'apikey': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpwaWtqamNiaWV2ZnB6ZWd1cG13Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY4NzU1NDEsImV4cCI6MjA5MjQ1MTU0MX0.HJxSSIcUSVrf5IAsjwnkf3eq0xZobchtlg1k_iFjW_g'
            },
            body: JSON.stringify({ accion: 'LOGOUT_ALL_USERS' })
        });
        ...
        if (data.success) {
            ...
            // Actualizar el localStorage para que el usuario actual también sea afectado
            localStorage.setItem('last_logout_check', '0');
            ...
            // Deslogear al usuario actual también
            if (typeof window.logout === 'function') {
                window.logout();
            }
        } else {
            throw new Error(data.message || 'Error al deslogear usuarios');
        }
    } catch (e) {
        console.error('[LOGOUT_ALL] Error:', e);
        Swal.fire({
            title: 'Error',
            text: e.message || 'No se pudo completar la operación',
            icon: 'error',
