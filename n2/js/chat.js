...
async function _readChatSheet(idNovedad = null) {
    try {
        if (!idNovedad) return [];
        const cleanId = String(Array.isArray(idNovedad) ? idNovedad[0] : idNovedad).trim();
        
        // Versión ultra-simplificada: Filtro directo
        const options = { 
        ...
        if (!data || data.length === 0) {
            ...
            if (data2 && data2.length > 0) return data2.map(_mapMsg);
            return [];
        }

        return data.map(_mapMsg);
    } catch (e) {
        console.error('[CHAT] Error crítico:', e);
        return [];
    }
}
