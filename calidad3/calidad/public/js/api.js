...
// ── Clave anon pública — usa la de config.js para evitar redeclaración ──
// SUPABASE_KEY ya está declarada en config.js
...
/**
 * Obtiene el listado de plantas - usa edge function personas
 */
async function fetchPlantasData(options = {}) {
    try {
        ...
        const response = await fetch(`${getFunctionsUrl()}/personas`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            ...
            console.error('[API] Error al listar plantas:', response.status, errorText);
            return [];
        }

        const result = await response.json();
        console.log('[API] Plantas obtenidas:', result);
        return result.data || [];
    } catch (e) {
        console.error('[API] Error en fetchPlantasData:', e);
        return [];
    }
}

/**
 * Obtiene usuarios - usa edge function personas
 */
async function fetchUsuariosData() {
    try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            console.error('[API] Error al listar usuarios:', response.status, errorText);
            return [];
        }

        const result = await response.json();
        console.log('[API] Usuarios obtenidos:', result);
        return result.data || [];
    } catch (e) {
        console.error('[API] Error al listar usuarios:', e);
        return [];
    }
}
...
/**
 * Obtiene reportes de auditoría filtrando por OP/Lote (consulta bajo demanda).
 * Usa la Edge Function exclusiva `consulta-reportes` para máxima velocidad.
 * Estrategia en el servidor: eq exacto → ilike parcial como fallback.
 */
async function fetchReportesByLote(lote) {
    if (!lote || !lote.trim()) return [];
    ...
    // ── Llamar a la Edge Function exclusiva `consulta-reportes` ──
    const resp = await fetch(`${getFunctionsUrl()}/consulta-reportes`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'apikey': SUPABASE_KEY,
        ...
        body: JSON.stringify({
            lote: loteTrimmed,
            productora: sessionUser?.ID_PRODUCTORA || null
        })
    });
    ...
    if (!resp.ok) {
        ...
        throw new Error(`[consulta-reportes] ${msg}`);
    }

    const resJson = await resp.json();
    if (!resJson.success) {
        throw new Error(resJson.error || 'Error desconocido en consulta-reportes');
    }

    console.log(`[API] consulta-reportes: ${resJson.meta?.total ?? 0} reportes, estrategia: ${resJson.meta?.estrategia}, ${resJson.meta?.tiempo_ms}ms`);

    return _normalizeSupabaseData(resJson.reportes || [], 'reportes');
}
