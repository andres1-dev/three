...
async function obtenerPerfil(supabaseClient: any, userId: string) {
  ...
  // Si no existe perfil, retornar estructura vacía
  if (!data) {
    return {
      success: true,
      data: {
...
async function actualizarPerfil(supabaseClient: any, userId: string, payload: PerfilPayload) {
  ...
  if (payload.fecha_contratacion !== undefined) updates.fecha_contratacion = payload.fecha_contratacion
  if (payload.sede !== undefined) updates.sede = payload.sede
  if (payload.division !== undefined) updates.division = payload.division
  
  if (payload.id_productora !== undefined) updates.id_productora = payload.id_productora
  ...
  if (payload.fecha_nacimiento !== undefined) updates.fecha_nacimiento = payload.fecha_nacimiento
  ...
  if (existente) {
    ...
    return { success: true, message: 'Perfil actualizado correctamente', data }
  } else {
    ...
    return { success: true, message: 'Perfil creado correctamente', data }
  }
}
