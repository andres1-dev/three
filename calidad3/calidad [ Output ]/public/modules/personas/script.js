...
(function () {
    ...
    function _normUser(u){
        return {
            _type:'USER',
            id:    String(u.id||u.auth_user_id||''),  // UUID para identificar internamente
            nombre:u.USUARIO||u.usuario||u.full_name||u.email||'Sin nombre',
            correo:u.CORREO||u.EMAIL||u.email||'—',
            tel:   u.TELEFONO||u.telefono||u.phone||'',
            rol:   u.ROL||u.role||'GUEST',
            prod:  u.PRODUCTORA||u.productora||'',
            id_prod:u.ID_PRODUCTORA||u.id_productora||null,
            pass:  '',  // NUNCA mostrar contraseñas
            firma: u.FIRMA_SVG||u.firma_svg||null,
            email_copia: u.EMAIL_COPIA||u.email_copia||false,
            cedula: u.cedula||'',  // La cédula real del usuario
            auth_user_id: u.id||u.auth_user_id||'', // UUID guardado para operaciones
        };
    }
    ...
    function _openDetail(item) {
        ...
        const displayId = item._type === 'USER' ? (item.cedula || 'Sin cédula') : item.id;
        ...
        el('p-sheet-body').innerHTML = `
            ...
            <div class="p-section">
                ...
                <div class="p-row"><span class="p-row-label">${item._type==='USER'?'Cédula':'ID'}</span><span class="p-row-value">${displayId}</span></div>
            ...
            ${item._type==='USER'&&item.prod&&item.prod!=='—'&&item.prod?`
            <div class="p-section">
    ...
    function _openEdit(item) {
        ...
        el('p-sheet-body').innerHTML = `
            ...
            <div style="padding:12px 20px 0">
                ${!isPlant?`
                <div class="p-field"><div class="p-field-label">Cédula</div>
                    <input class="p-field-input" id="pe-cedula" value="${item.cedula||''}" placeholder="Ingrese cédula"></div>
                <div class="p-field"><div class="p-field-label">Nombre Completo</div>
                    <input class="p-field-input" id="pe-nombre" value="${item.nombre}" placeholder="Nombre completo"></div>
                `:`
                <div class="p-field"><div class="p-field-label">NIT/ID</div>
                ...
                <div class="p-field"><div class="p-field-label">Nombre Taller</div>
                    <div class="p-field-wrap">
                        <input class="p-field-input" id="pe-nombre" value="${item.nombre}" disabled style="padding-right:36px">
                        <button class="p-field-lock" id="pe-lock-n">${_svg('<rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>')}</button>
                    </div></div>
                `}
                ...
                <div class="p-field"><div class="p-field-label">Teléfono</div>
                    <input class="p-field-input" id="pe-tel" type="tel" value="${item.tel!=='—'&&item.tel?item.tel:''}" placeholder="Ej: 3001234567"></div>
                ${isPlant?`<div class="p-field"><div class="p-field-label">Dirección</div>
                    <input class="p-field-input" id="pe-dir" value="${item.dir!=='—'?item.dir:''}" placeholder="Dirección completa"></div>`:''}
        ...
        // Limpiar el campo de contraseña después de un tick (evitar autocompletado)
        setTimeout(() => {
            const passInput = el('pe-pass');
            if (passInput) {
                passInput.value = '';
            }
        }, 100);
    ...
    async function _saveEdit(item) {
        ...
        // Para usuarios, usar siempre el auth_user_id (UUID) como identificador
        const userId = item.auth_user_id || item.id;
        
        const payload = {
            accion:      isPlant?'ACTUALIZAR_PLANTA':'UPDATE_USER',
            id:          userId,  // UUID de Auth para identificar
            cedula:      !isPlant ? (el('pe-cedula')?.value||'').trim() : null,  // Cédula editable
            nuevoId:     isPlant&&(el('pe-id')?.value||'').trim()!==item.id?(el('pe-id')?.value||'').trim():null,
        ...
        console.log('[Personas] Guardando usuario:', payload);
