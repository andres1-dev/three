...
function _startGuestPoll() {
    ...
    console.log('[CHAT-GUEST] 🔴 Iniciando Realtime para GUEST');
    ...
    window._guestBadgeChannel = sb
        ...
        .on('postgres_changes', { 
            ...
            console.log('[CHAT-GUEST] 💬 Nuevo mensaje detectado');
            _pollGuestChats();
        })
        .subscribe((status) => {
            if (status === 'SUBSCRIBED') {
                console.log('[CHAT-GUEST] ✅ Realtime conectado');
            } else if (status === 'CHANNEL_ERROR') {
                console.error('[CHAT-GUEST] ❌ Error en Realtime');
            }
        });
}
...
async function _pollGuestChats() {
    ...
    try {
        console.log('[CHAT-GUEST] Consultando mensajes para', ids.length, 'novedades');
        ...
        if (!response.ok) {
            console.error('[CHAT-GUEST] Error HTTP:', response.status);
            return;
        }
        ...
        if (!result.success) {
            console.error('[CHAT-GUEST] Error:', result.error);
            return;
        }
        ...
        console.log('[CHAT-GUEST] Mensajes recibidos:', filteredMessages.length);
        ...
        for (const id of ids) {
            ...
            if (lastMsg.rol !== 'GUEST' && lastMsg.ts !== _guestChatSeen[id]) {
                console.log('[CHAT-GUEST] Nuevo mensaje del operador para novedad', id);
