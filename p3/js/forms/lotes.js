...
function handleLoteSearch() {
    ...
    const filtered = currentLots.filter(
        (lot) => lot.LOTE && lot.LOTE.toLowerCase().includes(query),
    );
...
function verificarRegistroPlanta(plantaNombre) {
    ...
    if (!tieneDatos) {
        ...
        // Mostrar sección de actualización y Ocultar Acciones
        DOM.accionesSelect().value = 'ACTUALIZAR_DATOS';
        ...
        console.log(`[lotes] Planta "${plantaNombre}" ya registrada. Saltando formulario.`);
