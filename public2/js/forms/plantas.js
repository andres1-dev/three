...
function confirmarDireccion() {
    ...
    if (!tipoVia || !numPrincipal) {
        Swal.fire({
            ...
            text: 'Debes ingresar al menos el Tipo de Vía y el Número Principal',
    ...
    if (!direccionInput || !direccionInput.value.trim()) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'No se pudo construir la dirección. Verifica los datos ingresados.',
            confirmButtonText: 'Entendido'
        });
        return;
    }
...
function parsearDireccion(direccion) {
    ...
    console.log('[parsearDireccion] Parseando dirección:', direccion);
    ...
    if (match) {
        console.log('[parsearDireccion] Match encontrado:', match);
        ...
        console.log('[parsearDireccion] Campos llenados correctamente');
    } else {
        console.warn('[parsearDireccion] No se pudo parsear la dirección, intentando parseo simple');
        ...
        if (simpleMatch) {
            ...
            console.log('[parsearDireccion] Parseo simple exitoso');
        } else {
            // Si no se puede parsear, poner toda la dirección en complemento
            console.warn('[parsearDireccion] No se pudo parsear, poniendo todo en complemento');
...
async function handleActualizarDatosSubmit(e) {
    ...
    if (constructor && constructor.style.display !== 'none') {
        ...
        Swal.fire({
            icon: 'warning',
            title: 'Dirección sin confirmar',
            text: 'Debes confirmar la dirección antes de guardar los datos',
            confirmButtonText: 'Entendido'
        });
    ...
    const camposObligatorios = [
        { id: 'direccionPlanta', nombre: 'Dirección' },
        { id: 'telefonoPlanta', nombre: 'Teléfono' },
        { id: 'emailPlanta', nombre: 'Correo Electrónico' },
        { id: 'departamentoPlanta', nombre: 'Departamento' },
        { id: 'ciudadPlanta', nombre: 'Ciudad' },
        { id: 'barrioPlanta', nombre: 'Barrio' },
        { id: 'checkPoliticaDatos', nombre: 'Política de Datos', tipo: 'checkbox' },
        { id: 'checkNotificaciones', nombre: 'Notificaciones', tipo: 'checkbox' }
    ];
    ...
    camposObligatorios.forEach(campo => {
        ...
        if (campo.tipo === 'checkbox') {
            esValido = elemento.checked;
        } else {
            // Para barrio, verificar si está en modo manual
            if (campo.id === 'barrioPlanta') {
                const barrioManual = document.getElementById('barrioPlantaManual');
                if (barrioManual && barrioManual.style.display !== 'none') {
                    esValido = barrioManual.value.trim() !== '';
                    if (!esValido) {
                        barrioManual.style.border = '2px solid #ef4444';
                        barrioManual.style.boxShadow = '0 0 0 3px rgba(239, 68, 68, 0.1)';
                        if (!primerCampoConError) primerCampoConError = barrioManual;
                    }
                    return;
                }
            }
    ...
    // Si hay errores, mostrar alerta y hacer scroll al primer campo con error
    if (errores.length > 0) {
        Swal.fire({
            icon: 'error',
            title: 'Campos incompletos',
            html: `Por favor completa los siguientes campos obligatorios:<br><br><strong>${errores.join('<br>')}</strong>`,
            confirmButtonText: 'Entendido'
        });

        if (primerCampoConError) {
            primerCampoConError.scrollIntoView({ behavior: 'smooth', block: 'center' });
            setTimeout(() => primerCampoConError.focus(), 500);
        }
    ...
    if (typeof currentUser !== 'undefined' && currentUser && currentUser.ROL === 'GUEST') {
        ...
        console.log('[handleActualizarDatosSubmit] localStorage actualizado');
    }
