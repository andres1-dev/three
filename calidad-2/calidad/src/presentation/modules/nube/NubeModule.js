            ...
            btnTrigger.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (uploadType === 'EXTENSIONES') {
                        ...
                        if (!val.valid) {
                            console.error('[NubeModule][PasteButton] Posiciones inválidas Extensiones:', parsed.headers, val.error);
                            Toast.error(val.error, 4500);
                            return;
                        }
    ...
    _handleIncomingPaste(clipboardData) {
        try {
            ...
            if (uploadType === 'CONFECCION') {
                ...
                const val = validateClipboardPositions(parsed.headers, parsed.grid);
                if (!val.valid) {
                    console.error('[NubeModule][Paste] Posiciones inválidas Extensiones:', parsed.headers, val.error);
                    Toast.error(val.error, 4500);
                    return;
                }
...
// ── 5. INGESTA DE ARCHIVOS (Excel / CSV) ───────────────────────────────
    ...
    async _processConfeccionMatrix(matrix, fileName) {
        ...
        this.confeccionData = mapped;
        this.dataMode = 'confeccion';
        this.sourceName = fileName;
        this._renderMasterGrid(mapped, 'CONFECCION');
        this._updateUIState(mapped.length);
        Toast.success('Confección cargada: ' + mapped.length + ' registro(s).');
    }
    ...
    async _processProcesosMatrix(matrix, fileName) {
        ...
        this.procesosData = mapped;
        this.dataMode = 'procesos';
        this.sourceName = fileName;
        this._renderMasterGrid(mapped, 'PROCESOS');
        this._updateUIState(mapped.length);
        Toast.success('Procesos cargados: ' + mapped.length + ' registro(s).');
    }
        ...
        reader.onload = (e) => {
            try {
                ...
                if (uploadType === 'CONFECCION') {
                    ...
                    this._renderMasterGrid(mapped, 'CONFECCION');
                    this._updateUIState(mapped.length);
                    Toast.success('CSV Confección procesado: ' + mapped.length + ' registro(s).');
                } else if (uploadType === 'PROCESOS') {
                    const val = validateMasterHeaders(parsed.headers, 'PROCESOS');
                    if (!val.valid) {
                        Toast.error(val.error, 4500);
                    ...
                    this._renderMasterGrid(mapped, 'PROCESOS');
                    this._updateUIState(mapped.length);
                    Toast.success('CSV Procesos procesado: ' + mapped.length + ' registro(s).');
                } else if (uploadType === 'EXTENSIONES') {
                    const val = validateClipboardPositions(parsed.headers, parsed.grid);
                    if (!val.valid) {
                        Toast.error(val.error, 5000);
    ...
    async _cargarResumen() {
        try {
            ...
            // Cargar datos de Confección y Procesos desde tabla master.
            // Para ADMIN/MODERATOR sin productora seleccionada se consulta sin filtro
            // para mostrar el resumen global de todas las productoras.
            const role = String(this._userRole()).toUpperCase();
            const isAdmin = role === 'ADMIN' || role === 'MODERATOR' || role === 'SUPERADMIN';
            ...
            console.debug('[NubeModule][Resumen] Confección:', confeccion.length, 'Procesos:', procesos.length, 'filtro:', filtroProductora || 'TODAS');
            ...
            // Solo actualizar confeccionData/procesosData si hay productora activa
            // (para no mezclar datos de todas las productoras en el estado local)
            if (this.idProductora) {
