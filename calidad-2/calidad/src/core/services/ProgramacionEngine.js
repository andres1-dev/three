...
export function detectUploadType(headers) {
    ...
    // 2) Procesos: si no es confección pero tiene estructura base de master
...
export function detectProductoraDesdeCuento(rows, productoras) {
    ...
    // 1. Calcular la moda del campo cuento (valor más frecuente, normalizado)
    const freq = new Map();
    for (const r of rows) {
        const raw = String(r.cuento || '').trim();
        if (!raw) continue;
        // Normalizar: quitar "S2" al final, colapsar espacios, mayúsculas
        const norm = raw.toUpperCase().replace(/\s*S2\s*$/i, '').replace(/\s+/g, ' ').trim();
    ...
    // Debe ser al menos el 40% de las filas para considerarse representativo
    if (modaCount / rows.length < 0.4) return null;

    // 2. Mapa de términos clave de CUENTO → fragmento de búsqueda en productoras
    const CUENTO_MAP = [
    ...
    for (const entry of CUENTO_MAP) {
        if (entry.terms.some(t => modaKey.includes(t))) {
    ...
    const found = productoras.find(p => {
        const nombre = String(p.productora || p.nombre_corto || p.nombre || '').toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        const target = searchFor.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        return nombre.includes(target);
    });
