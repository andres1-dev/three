...
/**
 * Muestra el mapa con las coordenadas dadas.
 */
...
/**
 * Solicita la ubicación GPS en tiempo real (coordenadas 100% frescas, sin caché).
 * Se ejecuta al abrir el formulario de Calidad o al presionar refrescar/activar.
 */
function requestCalidadLocation() {
    ...
    // Si el usuario desactivó manualmente el toggle → bloquear módulo
    if (pref === 'disabled') {
    ...
    if (!navigator.geolocation) {
        mapaCard.innerHTML = `<span><i class="fas fa-exclamation-triangle me-2 text-warning"></i> Geolocalización no soportada en este navegador.</span>`;
        locInput.value = 'No soportado';
        return;
    }

    // Limpiar cualquier residuo de caché previa
    try { localStorage.removeItem('gps_coords_storage'); } catch (_) {}

    mapaCard.innerHTML = `
        <div style="text-align:center; padding:1.8rem; color:#64748b;">
            <i class="fas fa-spinner fa-spin me-2" style="color:#3b82f6; font-size:1.4rem;"></i>
            <div style="font-weight:600; font-size:0.9rem; margin-top:8px; color:#1e293b;">Obteniendo ubicación GPS en tiempo real...</div>
            <small style="color:#94a3b8;">Capturando coordenadas frescas del dispositivo</small>
        </div>
    `;
            ...
            if (error.code === error.PERMISSION_DENIED) {
                ...
                mapaCard.innerHTML = `
                    <div style="text-align:center; padding:1.5rem;">
                        <i class="fas fa-map-marker-alt" style="font-size:2rem; color:#f59e0b; margin-bottom:10px;"></i>
                        <p style="margin:0 0 10px; color:#475569; font-size:0.85rem; font-weight:600;">
                            No se pudo obtener la ubicación GPS fresca.<br>
                            <small style="color:#94a3b8; font-weight:400;">Verifica que el GPS y los datos de tu dispositivo estén activos.</small>
                        </p>
                        <button type="button" onclick="activarGpsManual()" style="
                        ...
                        ">
                            <i class="fas fa-location-crosshairs me-1"></i> Reintentar Captura GPS
                        </button>
                    </div>
                `;
            }
...
/**
 * Llamado desde el botón de refrescar o reintentar.
 */
