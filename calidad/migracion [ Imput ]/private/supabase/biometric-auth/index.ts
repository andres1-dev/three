...
// Función para obtener la MASTER_KEY desde diferentes fuentes
async function getMasterKey(): Promise<string> {
  // 1. Primero intentar desde variable de entorno (para desarrollo/local)
  const envKey = Deno.env.get('VAULT_MASTER_KEY');
  if (envKey) {
    console.log('Usando MASTER_KEY desde variable de entorno');
    return envKey;
  }

  // 2. Para producción, usar Supabase Secrets Manager o Deno KV
  try {
    // Opción A: Usar Deno KV (recomendado para Deno Deploy)
    const kv = await Deno.openKv();
    const secret = await kv.get<string>(["vault", "master_key"]);
    
    if (secret.value) {
      console.log('Usando MASTER_KEY desde Deno KV');
      return secret.value;
    }
    
    // Si no existe, crear una nueva MASTER_KEY
    const newKey = await generateSecureMasterKey();
    await kv.set(["vault", "master_key"], newKey);
    console.log('Generada nueva MASTER_KEY y guardada en KV');
    return newKey;
    
  } catch (kvError) {
    console.error('Error accediendo a Deno KV:', kvError);
    throw new Error('No se pudo obtener la MASTER_KEY de ninguna fuente segura');
  }
}

// Generar una MASTER_KEY segura
async function generateSecureMasterKey(): Promise<string> {
  // Generar una clave ECDSA P-256 segura (como la que tenías hardcodeada)
  ...
  // Exportar la clave pública como JWK y luego a base64
  const publicKey = await crypto.subtle.exportKey("spki", keyPair.publicKey);
  const base64Key = btoa(String.fromCharCode(...new Uint8Array(publicKey)));
...
async function decryptText(encryptedBase64: string, masterKey: string): Promise<string> {
...
serve(async (req) => {
  ...
  try {
    // Obtener la MASTER_KEY de manera segura al inicio
    ...
    if (action === 'enroll') {
      ...
      const decryptedPassword = await decryptText(vaultData.encrypted_password, MASTER_KEY);
