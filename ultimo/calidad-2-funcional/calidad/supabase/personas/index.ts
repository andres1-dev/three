...
/**
 * Normaliza un número telefónico al formato internacional E.164 (+57...)
 * Prohíbe más de 10 dígitos y quita cualquier caracter no numérico (como el signo +).
 */
function normalizarTelefonoE164(telefono: string | undefined | null): string | null {
...
async function listarUsuarios(supabaseClient: any) {
    try {
        ...
        const users = (authData?.users || []).map((authUser: any) => {
            ...
            return {
                // Identificadores
                ID_USUARIO: perfil?.cedula || meta.cedula || '',
                id: authUser.id,
...
async function crearUsuario(supabaseClient: any, payload: PersonaPayload) {
    ...
    // Normalizar teléfono (máximo 10 dígitos limpios para metadatos y formato E.164 para Auth)
    const phoneClean = telefono ? String(telefono).replace(/\D/g, '').slice(0, 10) : ''
    const phoneE164 = normalizarTelefonoE164(phoneClean)

    // Crear usuario en Auth (tanto en la columna nativa phone como en user_metadata)
    ...
    const { data: authData, error: authError } = await adminClient.auth.admin.createUser(createPayload)
...
async function actualizarUsuario(supabaseClient: any, payload: PersonaPayload) {
    ...
    // Normalizar teléfono si viene en el payload
    const phoneClean = telefono !== undefined ? String(telefono).replace(/\D/g, '').slice(0, 10) : undefined
    const phoneE164 = phoneClean !== undefined ? normalizarTelefonoE164(phoneClean) : undefined
    ...
    if (phoneClean !== undefined) metadataUpdates.phone = phoneClean
    ...
    try {
        ...
        if (phoneClean !== undefined) complementUpdates.phone = phoneClean
