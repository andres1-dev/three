...
async function _sendMsg(mensaje, imagenData = null) {
    ...
    try {
        ...
        const payload = {
            ...
            productora: window.currentUser?.ID_PRODUCTORA || '0',
        ...
        ">
            ...
            <div style="background:linear-gradient(135deg,#3b82f6,#6366f1);padding:14px 16px;display:flex;align-items:center;gap:10px;">
                ...
                <div style="flex:1;min-width:0;">
                    <div style="font-weight:800;font-size:0.88rem;color:white;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">Chat — Novedad ${_chatNovedadId || 'S/N'}</div>
...
function _startChatPoll() {
    ...
    // Canal de base de datos filtrado exclusivamente para esta novedad
    window._chatActiveChannel = sb.channel(`chat-room-${idNov}`)
            .on('postgres_changes', { 
                event: '*',
                schema: 'public',
                table: 'chat',
                filter: `id_novedad=eq.${idNov}`
            }, payload => {
                ...
                const evType = (payload.eventType || payload.event || '').toUpperCase();
                
                if (evType === 'INSERT') {
                    const myName = (currentUser?.PLANTA || currentUser?.USUARIO || '').trim().toLowerCase();
                    const msgAutor = (newMsg.autor || newMsg.AUTOR || '').trim().toLowerCase();
                    const isMyOwn = myName && msgAutor && (msgAutor === myName);
                    
                    // Si el mensaje NO es mío, lo inyecto instantáneamente en la pantalla
                    if (!isMyOwn) {
                        const mappedMsg = _mapMsg(newMsg);
                        _appendBubble(mappedMsg, true);
                        ...
                        // Marcar como leído
                        if (typeof _markChatSeen === 'function') _markChatSeen(idNov, mappedMsg.ts);
                    }
                }
                
                // Sincronización completa en background (con retraso para evitar race-condition del read replica)
                setTimeout(_loadAndRender, 600);
            })
            .subscribe((status) => {
                if (status === 'CLOSED' || status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
                    // Reconexión automática si el canal se cae
                    setTimeout(_startChatPoll, 2000);
                }
            });
}
...
async function _pollChatBadges() {
    try {
        ...
        (allMessages || []).forEach(r => {
            const novId = String(r.ID_NOVEDAD || r.id_novedad || '').trim();
            if (!novId) return;
            
            // Guardar siempre el más reciente (sobrescribe)
            latestGuestByNov[novId] = {
                id: r.ID_MSG || r.id_msg,
                rol: r.ROL || r.rol,    // ROL contiene el rol (GUEST/ADMIN)
                autor: r.AUTOR || r.autor, // AUTOR contiene el nombre real
                mensaje: r.MENSAJE || r.mensaje,
                ts: r.TS || r.ts,
                lote: r.LOTE || r.lote || ''
            };
...
function _addOperatorChatNotif(idNovedad, msg, lote, planta) {
    ...
    // Si el chat está abierto en esta ventana, no disparar notificaciones de fondo
    if (_chatNovedadId === idNovedad) {
        return;
    }
...
function _startGuestPoll() {
    ...
    window._guestBadgeChannel = sb
        ...
        .on('postgres_changes', { 
            ...
            const myRol = currentUser?.ROL || 'GUEST';
            const msgRol = r.rol || r.ROL || '';
            // Si el mensaje lo enviamos nosotros o el chat ya está abierto, no notificar por campana
            if (msgRol === myRol || _chatNovedadId === novId) return;

            const nov = _guestNovedades.find(n => n.ID_NOVEDAD === novId);
            if (nov) {
                const lastMsg = {
                    id: r.id_msg || r.ID_MSG,
                    rol: msgRol,
...
async function _pollGuestChats() {
    ...
    try {
        ...
        const filteredMessages = allMessages.filter(r => ids.includes(r.ID_NOVEDAD || r.id_novedad));
...
function _showChatToast(novIdStr, msg) {
    ...
    toast.innerHTML = `
        ...
        <div style="flex:1;min-width:0;">
            <div style="font-weight:800;font-size:0.8rem;color:#1e293b;margin-bottom:2px;">
                Nuevo mensaje — Novedad ${novIdStr}
            </div>
