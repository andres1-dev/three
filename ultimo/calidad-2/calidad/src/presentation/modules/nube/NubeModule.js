...
export class NubeModule {
    constructor({ router, guardarProgramacionUseCase, listarProgramacionUseCase, getProductorasNubeUseCase, resumenProgramacionUseCase, syncConfeccionUseCase, syncProcesosUseCase, listarMasterUseCase }) {
        ...
        // Interfaz limpia: no se muestran tablas de ningún tipo con los datos
        // cargados (hoja editable, lotes, JSON, grid master). Los datos viven
        // en memoria y se gestionan desde la ventana emergente de acciones.
        this._uiLimpia = true;
    ...
    _render() {
        this.container.innerHTML = `
            ...
            <div class="nube-real" id="nube-real" style="display:none; flex-direction:column; flex:1;">
                ...
                <div class="nube-body">
                    <!-- Interfaz limpia: sin botones fijos de Asentar/Limpiar.
                         Las acciones se realizan desde la ventana emergente
                         que aparece al cargar datos. -->
    ...
    _bindEvents() {
        ...
        const toggleHandler = () => {
            if (sel?.disabled) {
                Toast.info('Productora bloqueada — clic en el candado para desbloquear');
                return;
            }
    ...
    _syncProductoraUI() {
        ...
        // El candado siempre visible (permite desbloquear/bloquear en cualquier estado)
        if (btnClearProd) btnClearProd.style.display = 'inline-flex';
    ...
    async _cargarProductoras() {
        ...
        if (foundUser) {
            ...
            // Se desbloquea con clic en el candado 🔒 y se vuelve a bloquear al elegir.
            this._setProductoraBloqueada(true);
            this._syncProductoraUI();
            this._renderExcelGrid();
            this._updateUIState(this.rawGrid.length);
    ...
    _updateUIState(count) {
        if (count === undefined || count === null) count = this.rawGrid.length;

        const hasExt = count > 0 || this.jsonData.length > 0;
        const hasMaster = this.confeccionData.length > 0 || this.procesosData.length > 0;
        const hasAny = hasExt || hasMaster;
        const canAsentar = hasAny && !!this.idProductora;

        const btnClear = this.$('btnNubeClear');
        const btnAsentar = this.$('btnNubeAsentar');

        if (btnClear) btnClear.disabled = !canAsentar;
        if (btnAsentar) btnAsentar.disabled = !canAsentar;
    }
    ...
    _applyViewMode() {
        ...
        const hasAny = this.jsonData.length > 0 || this.rawGrid.length > 0 || this.confeccionData.length > 0 || this.procesosData.length > 0;

        // Siempre vista lotes: zona Excel visible solo cuando no hay datos
        if (zone) zone.classList.toggle('hidden', hasAny);
        if (lotesBar) lotesBar.classList.toggle('hidden', !hasAny);
        if (lotesContainer) lotesContainer.classList.toggle('hidden', !hasAny);

        // Botones Asentar y Limpiar: visibles solo con datos
        ['btnNubeClear', 'btnNubeAsentar'].forEach(id => {
            const b = this.$(id);
...
/* ── ESTADO LIMPIO + VENTANA EMERGENTE DE ACCIONES ────────────────────── */

    _statsCarga(tipo) {
        if (tipo === 'EXTENSIONES') {
            const filas = this.rawGrid.length || 0;
            const ops = this.jsonData.length;
            const extensiones = this.jsonData.reduce((a, r) => a + (r.extensiones ? r.extensiones.length : 0), 0);
            return { filas, ops, extensiones, resumen: `${filas} fila(s) · ${ops} OP(s) · ${extensiones} extensiones` };
        }
        const data = tipo === 'CONFECCION' ? this.confeccionData : this.procesosData;
        return { filas: data.length, ops: 0, extensiones: 0, resumen: `${data.length} registro(s)` };
    }

    _renderEstadoLimpio(tipo) {
        const zone = this.$('nubeExcelZone');
        if (!zone) return;
        zone.classList.remove('hidden');
        const s = this._statsCarga(tipo);
        const prod = this._escapeHtml(this.productoraNombre || '—');
        const titulo = tipo === 'EXTENSIONES'
            ? 'Extensiones listas para asentar'
            : (tipo === 'CONFECCION' ? 'Confección lista para sincronizar' : 'Procesos listos para sincronizar');
        const chips = tipo === 'EXTENSIONES'
            ? `<span class="carga-chip"><strong>${s.filas}</strong> filas</span>
               <span class="carga-chip"><strong>${s.ops}</strong> OPs</span>
               <span class="carga-chip"><strong>${s.extensiones}</strong> extensiones</span>`
            : `<span class="carga-chip"><strong>${s.filas}</strong> registros</span>`;
        zone.innerHTML = `
            <div class="nube-carga-lista">
                <div class="carga-check" aria-hidden="true">
                ...
                <div class="carga-titulo">${titulo}</div>
                <div class="carga-stats">${chips}
                    <span class="carga-chip carga-chip-prod">Productora: <strong>${prod}</strong></span>
                </div>
                <div class="carga-acciones">
                    <button id="btnCargaGestionar" class="nube-btn nube-btn-primary" type="button">Asentar</button>
                </div>
            </div>`;
        this.container.querySelector('#btnCargaGestionar')?.addEventListener('click', () => this._abrirModalAcciones(tipo));
    }

    _datosListos(tipo) {
        // Extensión viva del pipeline: si vienen filas crudas sin JSON, construirlo
        if (tipo === 'EXTENSIONES' && !this.jsonData.length && this.rawGrid.length) {
            this.jsonData = buildProgramacionJSON(this.rawHeaders, this.rawGrid, this.colMapping);
        }
        this._renderEstadoLimpio(tipo);
        const s = this._statsCarga(tipo);
        Toast.success(`${tipo === 'EXTENSIONES' ? 'Extensiones' : (tipo === 'CONFECCION' ? 'Confección' : 'Procesos')} listas: ${s.resumen}`);

        // Si aún no hay productora, desbloquear el candado para que pueda elegirla
        // (así el botón Asentar siempre tiene productora; evita el "no hace nada").
        if (!this.idProductora) {
            this._setProductoraBloqueada(false);
            Toast.info('Elija la productora (el candado está abierto) y luego pulse "Asentar".');
            return;
        }
        // Ventana emergente con confirmación de productora (extensiones) + Asentar/Limpiar
        this._abrirModalAcciones(tipo);
    }

    _abrirModalAcciones(tipo) {
        const esExt = tipo === 'EXTENSIONES';
        const s = this._statsCarga(tipo);
        const prod = this._escapeHtml(this.productoraNombre || '—');
        const titulo = esExt ? 'Asentar programación' : (tipo === 'CONFECCION' ? 'Sincronizar Confección' : 'Sincronizar Procesos');
        const chips = esExt
            ? `<span class="carga-chip"><strong>${s.filas}</strong> filas</span>
               <span class="carga-chip"><strong>${s.ops}</strong> OPs</span>
               <span class="carga-chip"><strong>${s.extensiones}</strong> extensiones</span>
            ...
            : `<span class="carga-chip"><strong>${s.filas}</strong> registros</span>
               <span class="carga-chip carga-chip-prod">Productora: <strong>${prod}</strong></span>`;
        const pregunta = esExt ? `
            <div class="carga-pregunta">
                <p>¿Es correcta la productora <strong>${prod}</strong>?</p>
                <p class="carga-pregunta-hint">Si no lo es, pulsa "Limpiar", desbloquea el candado, elige la productora correcta y vuelve a cargar los datos.</p>
            </div>` : '';
        const btnPrimario = esExt ? 'Sí, Asentar' : 'Sí, Sincronizar';
        const prodOk = !!this.idProductora;
        const bloqueProductora = esExt && !prodOk ? `
            <div class="carga-pregunta" style="border-color:#fca5a5; background:#fef2f2;">
                <p>No hay productora seleccionada.</p>
                <p class="carga-pregunta-hint">Desbloquea el candado 🔓 y elige la productora antes de asentar.</p>
            </div>` : pregunta;

        // Robusto: nunca dejar el botón "Asentar" sin acción visible.
        const btnAceptarHandler = () => {
            if (!this.idProductora) {
                Toast.error('Seleccione la productora — desbloquee el candado y elija antes de asentar.');
                this._setProductoraBloqueada(false);
                return;
            }
            try {
                Modal.close();
                this._submitAsentarModal();
            } catch (err) {
                Toast.error('Error al asentar: ' + (err?.message || err));
            }
        };
        ...
        Modal.open({
            title: titulo,
            contentHtml: `
                <div class="nube-modal-carga">
                    <div class="carga-stats carga-stats-modal">${chips}</div>
                    ${bloqueProductora}
                    <div class="carga-modal-acciones">
                        <button id="modalBtnAceptar" class="nube-btn nube-btn-primary" type="button">${btnPrimario}</button>
                        <button id="modalBtnLimpiar" class="nube-btn" type="button">Limpiar</button>
                    </div>
                </div>`,
            onOpen: (body) => {
                body.querySelector('#modalBtnAceptar')?.addEventListener('click', btnAceptarHandler);
                body.querySelector('#modalBtnLimpiar')?.addEventListener('click', () => {
                    Modal.close();
                    this._clearData();
                });
            }
    ...
    _renderExcelGrid() {
        const zone = this.$('nubeExcelZone');
        if (!zone) return;
        // Interfaz limpia: con datos cargados NO se muestra tabla editable.
        if (this._uiLimpia && (this.rawGrid.length || this.jsonData.length)) {
            this._renderEstadoLimpio('EXTENSIONES');
            return;
        }
...
// ── 4. VISTAS DERIVADAS (LOTES + JSON) ─────────────────────────────────
    ...
    _refreshDerivedViews() {
        // Interfaz limpia: sin vistas de lotes/JSON con los datos cargados.
        if (this._uiLimpia) return;
...
// ── 5. INGESTA DE ARCHIVOS (Excel / CSV) ───────────────────────────────
    ...
    _renderMasterGrid(data, type) {
        ...
        // Interfaz limpia: con datos cargados NO se muestra grid/tabla.
        if (this._uiLimpia && data && data.length) {
            this._renderEstadoLimpio(type);
            return;
        }
        ...
        const headers = ['ID Master', 'Referencia', 'Cantidad', 'Planta', 'Proceso', 'Fecha Salida', 'Fecha Entrega', 'Cuento', 'Género', 'Costo', 'Observaciones'];
        let html = '<div class="xls-sheet-container"><div class="xls-toolbar">';
        html += `<span class="xls-stat-badge">${data.length} registros</span>`;
        html += `<span class="xls-stat-badge">${type}</span>`;
        html += '</div><div class="xls-grid-scroll"><table class="xls-table"><thead><tr>';
        headers.forEach(h => html += `<th>${h}</th>`);
        html += '</tr></thead><tbody>';

        data.forEach(row => {
            html += '<tr>';
            html += `<td>${row.id_master || ''}</td>`;
            html += `<td>${this._escapeHtml(row.referencia || '')}</td>`;
            html += `<td>${row.cantidad || 0}</td>`;
            html += `<td>${this._escapeHtml(row.nombre_planta || '')}</td>`;
            html += `<td>${this._escapeHtml(row.proceso || '')}</td>`;
            html += `<td>${row.fecha_salida || ''}</td>`;
            html += `<td>${row.fecha_entrega || ''}</td>`;
            html += `<td>${this._escapeHtml(row.cuento || '')}</td>`;
            html += `<td>${this._escapeHtml(row.genero || '')}</td>`;
            html += `<td>${this._escapeHtml(row.costo || '')}</td>`;
            html += `<td>${this._escapeHtml(row.observaciones || '')}</td>`;
            html += '</tr>';
        });
        ...
        const btnAsentar = this.$('btnNubeAsentar');
        if (btnAsentar) btnAsentar.disabled = false;
    ...
    async _submitAsentarModal() {
        ...
        if (this.dataMode === 'confeccion' && this.confeccionData.length) {
            ...
            Toast.warning('No hay datos para asentar.');
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                Toast.success(result.message || 'Datos asentados correctamente.');
                await this._cargarResumen();
                this._clearData();
            } else {
                Toast.error(result.message || 'Error al asentar datos.');
            }
        } catch (err) {
            console.error('Error al asentar:', err);
            Toast.error('Error al asentar: ' + err.message);
        }
    }
    ...
    async _cargarResumen() {
        const role = String(this._userRole()).toUpperCase();
        const isAdmin = role === 'ADMIN' || role === 'MODERATOR' || role === 'SUPERADMIN';
        const filtroProductora = this.idProductora || (isAdmin ? '' : null);

        // Si el usuario no es admin y no tiene productora, no cargar master
        if (filtroProductora === null) return;

        const limitRows = isAdmin && !this.idProductora ? 2000 : 500;

        try {
            // ── VÍA RÁPIDA: RESUMEN_COMPLETO (UNA sola llamada, agregación SQL) ──
            ...
            if (data && Array.isArray(data.extensiones)) {
                // 1) Extensiones (desde RESUMEN_COMPLETO o del legacy)
                this._renderResumenSection('Extensiones', data.extensiones, 'nubeResumenTbodyExtensiones', 'nubeResumenEmptyExtensiones', 'badge-count-extensiones');

                // 2) Confección/Procesos
                if (data._legacy || !Array.isArray(data.confeccion) || !Array.isArray(data.procesos)) {
                    // Ruta legacy: traer master vía LISTAR_MASTER (2 llamadas)
                    const masterParams = { limit: isAdmin && !this.idProductora ? 5000 : 500 };
                    if (filtroProductora) masterParams.idProductora = filtroProductora;

                    const [confeccion, procesos] = await Promise.all([
                        this.listarMasterUseCase.execute({ tipo: 'CONFECCION', ...masterParams }).catch(() => []),
                        this.listarMasterUseCase.execute({ tipo: 'PROCESOS', ...masterParams }).catch(() => [])
                    ]);

                    const resumenConfeccion = this._buildResumenFromMasterRows(confeccion);
                    const resumenProcesos = this._buildResumenFromMasterRows(procesos.filter(r => r.proceso && r.proceso.toUpperCase() !== 'CONFECCION'));
                    this._renderResumenSection('Confección', resumenConfeccion, 'nubeResumenTbodyConfeccion', 'nubeResumenEmptyConfeccion', 'badge-count-confeccion');
                    this._renderResumenSection('Procesos', resumenProcesos, 'nubeResumenTbodyProcesos', 'nubeResumenEmptyProcesos', 'badge-count-procesos');

                    if (this.idProductora) {
                        this.confeccionData = confeccion;
                        this.procesosData = procesos;
                        this._updateUIState(this.confeccionData.length || this.procesosData.length || 0);
                    }
                } else {
                    // Vía rápida: vienen incluidos en la respuesta
                    this._renderResumenSection('Confección', data.confeccion, 'nubeResumenTbodyConfeccion', 'nubeResumenEmptyConfeccion', 'badge-count-confeccion');
                    this._renderResumenSection('Procesos', data.procesos, 'nubeResumenTbodyProcesos', 'nubeResumenEmptyProcesos', 'badge-count-procesos');

                    if (this.idProductora && data.rows) {
                        this.confeccionData = Array.isArray(data.rows.confeccion) ? data.rows.confeccion : [];
                        this.procesosData = Array.isArray(data.rows.procesos) ? data.rows.procesos : [];
                        this._updateUIState(this.confeccionData.length || this.procesosData.length || 0);
                    }
                }
                return;
            ...
            // ── FALLBACK: Edge Function aún sin RESUMEN_COMPLETO desplegada ──
            console.warn('[NubeModule][Resumen] RESUMEN_COMPLETO no disponible, usando ruta antigua:', err?.message || err);
            try {
                this._renderResumen({ resumen: [], ultima: null });
                ...
                console.error('Error al cargar resumen (fallback):', err2);
            }
        }
    }
