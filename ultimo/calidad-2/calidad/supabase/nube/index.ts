...
serve(async (req) => {
  ...
  try {
    ...
    // ── Resumen COMPLETO en UNA sola llamada (rápido y consistente) ─────────
    // Usa RPCs SQL nativos (nube_resumen_* / nube_listar_master) que agregan
    // por productora DENTRO de la base de datos: sin escanear miles de filas
    // en JS ni límites arbitrarios que truncaban resultados.
    ...
    if (accion === "RESUMEN_COMPLETO") {
      ...
      const lim = Number(payload.limitRows || 1000)

      const safe = async (fn: string, args: any) => {
        try {
          const { data, error } = await db.rpc(fn, args)
          if (error) throw error
          return data
        } catch (e: any) {
          console.warn(`[NUBE][RESUMEN_COMPLETO] RPC ${fn} no disponible: ${e?.message || e}`)
          return null
        }
      }

      const [ext, conf, proc, confRows, procRows] = await Promise.all([
        safe("nube_resumen_extensiones", { id_prod: idProd }),
        safe("nube_resumen_master", { tipo: "CONFECCION", id_prod: idProd }),
        safe("nube_resumen_master", { tipo: "PROCESOS", id_prod: idProd }),
        safe("nube_listar_master", { tipo: "CONFECCION", id_prod: idProd, lim }),
        safe("nube_listar_master", { tipo: "PROCESOS", id_prod: idProd, lim }),
      ])

      const extensiones = Array.isArray(ext) ? ext : []
      const confeccion = Array.isArray(conf) ? conf : []
      const procesos = Array.isArray(proc) ? proc : []
      ...
      return json({
        ...
        procesos,
        rows: {
          confeccion: Array.isArray(confRows) ? confRows : [],
          procesos: Array.isArray(procRows) ? procRows : [],
        },
        ultima,
      })
    }
