...
(function _renderNavInstant() {
    ...
    const html = `
        ...
        <div class="nav-user-area" style="display:flex;align-items:center;gap:6px;">
            <button class="btn-expand-view" id="btn-expand-view" onclick="toggleExpandView()" title="Expandir vista">
...
/* ── Expand View — expande/contrae los contenedores al 100% en PC ── */
function toggleExpandView() {
    const expanded = document.body.classList.toggle('view-expanded');
    ...
    if (btn) {
        ...
        btn.title = expanded ? 'Contraer vista' : 'Expandir vista';
        btn.style.color = expanded ? 'var(--color-primary)' : '';
    }
    // Persistir preferencia
    localStorage.setItem('sispro_view_expanded', expanded ? '1' : '0');
}

// Restaurar preferencia al cargar
(function _restoreExpandState() {
    if (localStorage.getItem('sispro_view_expanded') === '1') {
        document.body.classList.add('view-expanded');
        // Actualizar icono cuando el botón esté disponible
