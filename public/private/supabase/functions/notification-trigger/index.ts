...
async function handleNovedadStateChange(
  ...
  if (record.ID_USUARIO) {
    ...
    // Obtener todos los usuarios GUEST desde Auth (la tabla USUARIOS ya no existe)
    const { data: authData, error: authErr } = await supabase.auth.admin.listUsers();
    if (!authErr && authData && authData.users) {
      const guests = authData.users.filter((u: any) => 
        (u.user_metadata?.role === 'GUEST' || u.user_metadata?.ROL === 'GUEST')
      );
      recipients.push(...guests.map((g: any) => g.user_metadata?.id_usuario || g.id));
    }
  }
...
async function handleNewNovedad(supabase: any, record: any): Promise<any> {
  ...
  // Notificar a usuarios ADMIN y USER-P desde Auth (la tabla USUARIOS ya no existe)
  const { data: authData, error: authErr } = await supabase.auth.admin.listUsers();
  if (authErr || !authData || !authData.users) return null;

  const operators = authData.users.filter((u: any) => 
    ['ADMIN', 'USER-P'].includes(u.user_metadata?.role || u.user_metadata?.ROL)
  );

  if (operators.length === 0) return null;

  return {
    userIds: operators.map((o: any) => o.user_metadata?.id_usuario || o.id),
...
async function handleNewChatMessage(supabase: any, record: any): Promise<any> {
  ...
  if (rolAutor === 'GUEST') {
    // Notificar a operadores (ADMIN y USER-P) desde Auth (la tabla USUARIOS ya no existe)
    const { data: authData, error: authErr } = await supabase.auth.admin.listUsers();
    ...
    if (!authErr && authData && authData.users) {
      ...
      recipients.push(...operators.map((o: any) => o.user_metadata?.id_usuario || o.id));
    }
  } else {
