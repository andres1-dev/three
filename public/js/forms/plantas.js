...
async function handleActualizarDatosSubmit(e) {
    ...
    const nombrePlantaEl = document.getElementById('nombrePlanta');
    const nombrePlanta = nombrePlantaEl ? nombrePlantaEl.value : (window.currentUser?.PLANTA || window.currentUser?.planta || '');
    const emailPlanta  = document.getElementById('emailPlanta').value;

    const aceptaNotificaciones = checkNotif.checked;
    const aceptaPoliticaDatos = checkPolitica.checked;
    ...
    // 1. Actualizar objeto en memoria y en localStorage
    if (window.currentUser && window.currentUser.ROL === 'GUEST') {
        ...
        try {
            ...
            if (universalData) {
                universalData.EMAIL = emailPlanta;
                universalData.email = emailPlanta;
                universalData.TELEFONO = rawTelefono;
                universalData.telefono = rawTelefono;
                localStorage.setItem('busint_universal_plant', JSON.stringify(universalData));
            }
        } catch(e) {
            console.error('[GUEST Update] Error persisting to busint_universal_plant:', e);
        }
    }
    ...
    if (window.currentUser && window.currentUser.ID_PRODUCTORA) {
        payload.productora = window.currentUser.ID_PRODUCTORA;
    }
