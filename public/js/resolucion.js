...
const EDIT_CODIGOS_TALLAS_LIST = [
...
const EDIT_CODIGOS_COLORES_LIST = [
...
// ============================================================================
let globalNovedadesPromise = null;
let globalPlantasPromise = null;
    ...
    try {
        ...
        if (savedProdRaw && typeof window.currentUser === 'undefined') {
            ...
            if (savedProd && savedProd.ID_PRODUCTORA) {
                window.currentUser = { ROL: 'ADMIN', ID_PRODUCTORA: savedProd.ID_PRODUCTORA };
                mocked = true;
            }
        }
...
function handleFilter() {
    ...
    renderTabla(gsNovedades.filter(n => {
        if (!term) return true;
        const loteVal = n.id || n.ID || n.LOTE || '';
        return String(loteVal).toLowerCase().includes(term) ||
 ...
 */
function actualizarMaximoCodigoEdit(inputElement) {
