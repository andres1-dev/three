...
const FirmaTaller = {
    ...
    init(containerId) {
        ...
        function resizeInlineCanvas() {
            ...
            // Redibujar firma inline escalada proporcionalmente
            if (self._strokes && self._strokes.length > 0) {
                self.ctx.beginPath();
                self._strokes.forEach(stroke => {
                    if (stroke.length < 2) return;
                    self.ctx.moveTo(stroke[0].x * rect.width, stroke[0].y * rect.height);
                    for (let i = 1; i < stroke.length; i++) {
                        self.ctx.lineTo(stroke[i].x * rect.width, stroke[i].y * rect.height);
                    }
                });
                self.ctx.stroke();
            }
        ...
        function resizeFsCanvas() {
            ...
            // Restaurar los trazos si la pantalla giró o se redimensionó escalando los puntos normalizados
            if (self._fsStrokes && self._fsStrokes.length > 0) {
                self.fsCtx.beginPath();
                self._fsStrokes.forEach(stroke => {
                    if (stroke.length < 2) return;
                    self.fsCtx.moveTo(stroke[0].x * w, stroke[0].y * h);
                    for (let i = 1; i < stroke.length; i++) {
                        self.fsCtx.lineTo(stroke[i].x * w, stroke[i].y * h);
                    }
                });
                self.fsCtx.stroke();
            }
        ...
        this.canvas.addEventListener('mousedown', (e) => {
            ...
            self._currentStroke = [{ x: p.x / (rect.width || 1), y: p.y / (rect.height || 1) }];
        });
        ...
        this.canvas.addEventListener('mousemove', (e) => {
            ...
            if (self._currentStroke) {
                self._currentStroke.push({ x: p.x / (rect.width || 1), y: p.y / (rect.height || 1) });
            }
        });
        ...
        this.fsCanvas.addEventListener('mousedown', (e) => {
            ...
            const rect = self.fsCanvas.getBoundingClientRect();
            const isPortrait = window.innerHeight > window.innerWidth;
            const w = isPortrait ? rect.height : rect.width;
            const h = isPortrait ? rect.width : rect.height;
            self._fsCurrent = [{ x: p.x / (w || 1), y: p.y / (h || 1) }];
        });
        ...
        this.fsCanvas.addEventListener('mousemove', (e) => {
            ...
            const h = isPortrait ? rect.width : rect.height;
            if (self._fsCurrent) {
                self._fsCurrent.push({ x: p.x / (w || 1), y: p.y / (h || 1) });
            }
        });
        ...
        document.getElementById('saveFullscreenSigBtn').addEventListener('click', (e) => {
            ...
            // Asegurar que el canvas inline está correctamente dimensionado y redibujado
            resizeInlineCanvas();
    ...
    getSVG() {
        ...
        let scale = 1;
        if (sigW > 0.001 || sigH > 0.001) {
            const scaleX = maxUsefulW / (sigW || 0.001);
            const scaleY = maxUsefulH / (sigH || 0.001);
