...
const FirmaTaller = {
    ...
    init(containerId) {
        ...
        function redrawFs() {
            if (!self.fsCanvas || !self.fsCtx) return;
            self.fsCtx.clearRect(0, 0, self.fsCanvas.width, self.fsCanvas.height);
            if (!self._fsStrokes || self._fsStrokes.length === 0) return;
        ...
        document.getElementById('fullscreenFormSignatureBtn').addEventListener('click', (e) => {
            ...
            requestAnimationFrame(() => {
                setTimeout(() => {
                    resizeFsCanvas();
                    redrawFs(); // Mantener firma existente
                }, 80);
            });
        });
        ...
        document.getElementById('clearFullscreenSigBtn').addEventListener('click', (e) => {
            ...
            self.fsCtx.clearRect(0, 0, self.fsCanvas.width, self.fsCanvas.height);
            self._fsStrokes = [];
            // También limpiamos el inline para evitar inconsistencias
        ...
        document.getElementById('saveFullscreenSigBtn').addEventListener('click', (e) => {
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    icon: 'success',
                    title: 'Firma guardada temporalmente',
                    toast: true,
                    position: 'top-end',
        ...
        window.addEventListener('resize', () => {
            if (fsModal.style.display === 'flex') {
                resizeFsCanvas();
                redrawFs(); // Redibujar trazos para no perderlos al girar pantalla
            }
        });
    },
