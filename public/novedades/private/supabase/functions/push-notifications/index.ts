  ...
  try {
    ...
    switch (action) {
      ...
      case "send-to-role": {
        ...
        // Obtener usuarios del rol desde Auth (la tabla USUARIOS ya no existe)
        const { data: authData, error: authErr } = await supabaseClient.auth.admin.listUsers();
        if (authErr || !authData || !authData.users) throw new Error("Error al consultar usuarios en Auth");

        const usersWithRole = authData.users.filter((u: any) => 
          (u.user_metadata?.role === role || u.user_metadata?.ROL === role)
        );

        const userIds = usersWithRole.map((u: any) => u.user_metadata?.id_usuario || u.id);
