...
async function _pollNovedades() {
    try {
        console.log('[NOTIF] Polling novedades...');
        // No necesitamos fetchSecureConfig()
        const novedades = await fetchNovedadesData();
        console.log('[NOTIF] Novedades recibidas:', novedades.length);
...
function _processUpdates(novedades) {
    console.log('[NOTIF] Procesando actualizaciones, novedades:', novedades.length);
    const newNotifs = [];
    const isFirstLoad = !_initialLoadDone;
    console.log('[NOTIF] Primera carga:', isFirstLoad, 'Estados conocidos:', Object.keys(_lastKnownStates).length);
    ...
    novedades.forEach(nov => {
        ...
        if (estadoAnterior === undefined) {
            ...
            if (!isFirstLoad) {
                ...
                if (estadoActual === 'ELABORACION' || estadoActual === 'FINALIZADO') {
                    console.log('[NOTIF] Nueva novedad detectada:', id, estadoActual);
        ...
        if (estadoAnterior !== estadoActual) {
            console.log('[NOTIF] Cambio de estado detectado:', id, estadoAnterior, '→', estadoActual);
            if (
    ...
    console.log('[NOTIF] Nuevas notificaciones a agregar:', newNotifs.length);
...
function _addNotifications(items) {
    console.log('[NOTIF] Agregando notificaciones:', items.length);
    ...
    items.forEach(item => {
        ...
        if (_notifications.some(n => n.id === id)) {
            console.log('[NOTIF] Notificación duplicada, ignorando:', id);
            return;
        }
        
        console.log('[NOTIF] Nueva notificación:', id, item.estadoActual);
    ...
    if (hasNewNotifs) {
        console.log('[NOTIF] Reproduciendo sonido de estado...');
