...
const UIController = {
  ...
  showTracking(fileName) {
    ...
    document.getElementById('trackingFileName').textContent = fileName;
    document.getElementById('trackingStatus').className = 'tracking-status processing';
  ...
  completeProcess(results) {
    ...
    const totalTime = Object.keys(this.stepTimers)
      ...
      .reduce((sum, key) => {
        ...
        if (this.stepTimers[nextKey]) {
          return sum + (this.stepTimers[nextKey] - this.stepTimers[key]);
        }
    ...
    if (isSuccess) {
      ...
      statusDiv.querySelector('.status-text').textContent = 'Completado';
      ...
      statusDiv.querySelector('.status-text').textContent = 'Completado con errores';
