...
function initPushNotifications() {
    ...
    if (!window.supabase) {
        console.warn('[PUSH] Supabase no está inicializado');
        return;
    }

    // Obtener configuración de Supabase
    const SUPABASE_URL = 'https://doqsurxxxaudnutsydlk.supabase.co';
    const SUPABASE_ANON_KEY = window.supabase?.supabaseKey || '';

    if (!SUPABASE_ANON_KEY) {
        console.error('[PUSH] No se encontró la Supabase Anon Key');
        return;
    }
