...
// Clave localStorage para registros manuales (se define aquí para que esté disponible desde el init)
const _MANUAL_LS_KEY = 'liq_rodamiento_manuales_v1';

// Alias de nombres de plantas (nombreOriginal → nombreMostrado)
const _ALIAS_LS_KEY = 'liq_rodamiento_alias_plantas_v1';
let _aliasPlanta = {};   // cargado en init

function _cargarAliasLS() {
    try { _aliasPlanta = JSON.parse(localStorage.getItem(_ALIAS_LS_KEY) || '{}'); } catch(e) { _aliasPlanta = {}; }
}
function _guardarAliasLS() {
    try { localStorage.setItem(_ALIAS_LS_KEY, JSON.stringify(_aliasPlanta)); } catch(e) {}
}
/** Devuelve el nombre a mostrar (alias si existe, original si no) */
function _nombrePlanta(original) {
    return _aliasPlanta[original] || original;
}
...
async function initLiquidacionRodamiento() {
    try {
        ...
        // Restaurar registros manuales guardados en localStorage
        // Evitar duplicados si ya hay manuales en liqReportes (ej: recarga parcial)
        const manualesGuardados = _cargarManualesLS();
        if (manualesGuardados.length > 0) {
            // Solo agregar los que no estén ya en liqReportes
            const yaEnMemoria = liqReportes.filter(r => r._manual);
            if (yaEnMemoria.length === 0) {
                liqReportes.push(...manualesGuardados);
                console.log(`[Manuales] Restaurados ${manualesGuardados.length} registros desde localStorage`);
            }
        }

        // Restaurar alias de nombres de plantas
        _cargarAliasLS();
...
async function generarLiquidacion() {
    ...
    // Calcular rango de fechas general
    ...
    // Fechas de corte especiales: leer desde los inputs de los tabs (ya renderizados) o usar periodo general
    const corteAngeles     = { inicio: _parseIso(_leerFechaCorteGuardada('angeles-inicio')) || fechaInicio, fin: _parseIso(_leerFechaCorteGuardada('angeles-fin')) || fechaFin };
    const corteHacemosModa = { inicio: _parseIso(_leerFechaCorteGuardada('hm-inicio'))      || fechaInicio, fin: _parseIso(_leerFechaCorteGuardada('hm-fin'))      || fechaFin };

    // Filtrar reportes Supabase: líneas especiales usan su propio rango
    const reportesPeriodo = filtrarReportesPorPeriodoConCorte(liqReportes, fechaInicio, fechaFin, corteAngeles, corteHacemosModa);
    const aprobacionesPeriodo = filtrarReportesPorPeriodoConCorte(liqAprobaciones, fechaInicio, fechaFin, corteAngeles, corteHacemosModa);

    // Combinar todos los registros (liqReportes ya incluye los manuales agregados en memoria)
    const todosRegistros = [...reportesPeriodo, ...aprobacionesPeriodo];

    // Días a descontar por sección (leídos desde los inputs de cada tab)
    ...
    // Descuentos por sección — Universo usa el input general, especiales tienen el suyo propio en el tab
    const diasDescUniverso = fechasDescontadas.length;
    const diasDescHM       = parseInt(document.getElementById('dias-desc-hm')?.value || '0') || 0;
    const diasDescAngeles  = 0; // Ángeles nunca tiene descuento

    // Generar liquidación
    const liquidacion = procesarLiquidacion(todosRegistros, fechaInicio, fechaFin, diasDescUniverso, diasDescHM, diasDescAngeles);
...
/**
 * Filtra reportes respetando fechas de corte personalizadas por línea especial
 */
function filtrarReportesPorPeriodoConCorte(reportes, fechaInicio, fechaFin, corteAngeles, corteHacemosModa) {
    if (!reportes || reportes.length === 0) return [];
    ...
    return reportes.filter(rep => {
        ...
        const linea = (rep.LINEA || rep.linea || '').toUpperCase();

        if (linea === 'ANGELES') {
            return fechaRep >= corteAngeles.inicio && fechaRep <= corteAngeles.fin;
        }
        if (linea === 'HACEMOS MODA') {
            return fechaRep >= corteHacemosModa.inicio && fechaRep <= corteHacemosModa.fin;
        }
        return fechaRep >= fechaInicio && fechaRep <= fechaFin;
    });
}
 ...
 * diasDescUniverso: días a descontar para líneas Universo
 * diasDescHM:       días a descontar para HACEMOS MODA
 * diasDescAngeles:  días a descontar para ANGELES (siempre 0)
 */
function procesarLiquidacion(registros, fechaInicio, fechaFin, diasDescUniverso = 0, diasDescHM = 0, diasDescAngeles = 0) {
    const liquidacion = {
        lineas: {},
        totales: {
            visitas: 0,
    ...
    // Calcular total de visitas en líneas del Universo (no especiales) para el prorrateo
    ...
    // Descuento para Universo
    const descuentoUniverso = Math.round(diasDescUniverso * (350000 / 15));
    ...
    Object.keys(liquidacion.lineas).forEach(linea => {
        ...
        if (LIQ_CONFIG.LINEAS_ESPECIALES.includes(linea)) {
            let valorBase = linea === 'HACEMOS MODA' ? LIQ_CONFIG.VALOR_QUINCENA_HACEMOS_MODA : LIQ_CONFIG.VALOR_QUINCENA_ANGELES;
            // Aplicar descuento por sección: HM usa diasDescHM, Ángeles = 0 siempre
            const diasDesc = linea === 'HACEMOS MODA' ? diasDescHM : diasDescAngeles;
            const descuento = Math.round(diasDesc * (valorBase / 15));
            const valorQuincena = Math.max(0, valorBase - descuento);
    ...
    // Garantizar que todas las líneas conocidas existen aunque no tengan registros en el periodo
    // Así los tabs siempre muestran la tabla (vacía) en lugar de "sin visitas"
    const TODAS_LINEAS = [...LIQ_CONFIG.LINEAS_UNIVERSO, ...LIQ_CONFIG.LINEAS_ESPECIALES];
    TODAS_LINEAS.forEach(linea => {
        if (!liquidacion.lineas[linea]) {
            liquidacion.lineas[linea] = { plantas: {}, totales: { visitas: 0, valor: 0 } };
        }
    });
...
/**
 * Obtiene el nombre real de la auditora.
 * 1. Si es el usuario activo, usa currentUser.USUARIO
 * 2. Si no, busca en liqUsuarios por email → campo USUARIO o NOMBRE
 */
function obtenerAuditora(registros) {
    // 1. Usuario de sesión activa
    if (typeof currentUser !== 'undefined' && currentUser) {
        const nombre = currentUser.USUARIO || currentUser.PLANTA;
        if (nombre && !nombre.includes('@')) return nombre.toUpperCase();
    }

    if (!registros || registros.length === 0) return 'N/A';

    const primerRep = registros[0];
    const email = (primerRep.email || primerRep.EMAIL || primerRep.auditor || '').toLowerCase().trim();
    if (!email) return 'N/A';

    // 2. Buscar en liqUsuarios por correo → USUARIO o NOMBRE
    const userObj = liqUsuarios.find(u =>
        (u.CORREO || u.EMAIL || u.email || '').toLowerCase().trim() === email
    );

    if (userObj) {
        const nombre = userObj.USUARIO || userObj.NOMBRE || userObj.nombre;
        if (nombre) return nombre.toUpperCase();
    }

    // 3. Si el email coincide con el usuario activo, usar su nombre
    if (typeof currentUser !== 'undefined' && currentUser &&
        (currentUser.EMAIL || currentUser.CORREO || '').toLowerCase() === email) {
        const nombre = currentUser.USUARIO || currentUser.PLANTA;
        if (nombre) return nombre.toUpperCase();
    }

    // 4. Último recurso: mostrar el correo completo tal como está
...
function renderizarLiquidacion(liquidacion, fechaInicio, fechaFin, fechasDescontadas = []) {
    ...
    // Verificar si hay al menos alguna visita real (no solo líneas vacías garantizadas)
    const hayDatos = Object.values(liquidacion.lineas).some(l => l.totales.visitas > 0);
    if (!hayDatos) {
        container.innerHTML = `
    ...
    // Leer fechas de corte guardadas en los inputs del DOM anterior (si ya se había generado)
    // y convertirlas a Date para pasarlas al render de tabla
    const angelesInicioVal = _leerFechaCorteGuardada('angeles-inicio') || _isoFecha(fechaInicio);
    const angelesFinVal    = _leerFechaCorteGuardada('angeles-fin')    || _isoFecha(fechaFin);
    const hmInicioVal      = _leerFechaCorteGuardada('hm-inicio')      || _isoFecha(fechaInicio);
    const hmFinVal         = _leerFechaCorteGuardada('hm-fin')         || _isoFecha(fechaFin);

    // Convertir a Date para que obtenerDiasPeriodo genere las columnas correctas
    const hmInicio      = _parseIso(hmInicioVal)      || fechaInicio;
    const hmFin         = _parseIso(hmFinVal)         || fechaFin;
    const angInicio     = _parseIso(angelesInicioVal) || fechaInicio;
    const angFin        = _parseIso(angelesFinVal)    || fechaFin;
    ...
    let html = `
        ...
        <div class="tabs-content">
            <!-- PESTAÑA UNIVERSO: columnas = periodo general -->
            <div id="tab-universo" class="liq-tab-content">
                ${_renderControlesTab('universo', fechaInicio, fechaFin, null, null)}
                ${renderizarGrupoTab(liquidacion, 'universo', fechaInicio, fechaFin, fechasDescontadas)}
            </div>
            
            <!-- PESTAÑA HACEMOS MODA: columnas = rango HM -->
            <div id="tab-hacemos-moda" class="liq-tab-content" style="display: none;">
                ${_renderControlesTab('HACEMOS MODA', fechaInicio, fechaFin, hmInicioVal, hmFinVal)}
                ${renderizarGrupoTab(liquidacion, 'HACEMOS MODA', hmInicio, hmFin, fechasDescontadas)}
            </div>
            
            <!-- PESTAÑA ANGELES: columnas = rango Ángeles -->
            <div id="tab-angeles" class="liq-tab-content" style="display: none;">
                ${_renderControlesTab('ANGELES', fechaInicio, fechaFin, angelesInicioVal, angelesFinVal)}
                ${renderizarGrupoTab(liquidacion, 'ANGELES', angInicio, angFin, [])}
            </div>
        </div>
    `;
    ...
    // Mes/año del periodo seleccionado — el picker de fecha visita debe abrirse en ese mes
    const _mesPeriodo  = parseInt(document.getElementById('mesSelector').value);
    const _anioPeriodo = parseInt(document.getElementById('anioSelector').value);
    // defaultDate: primer día del mes del periodo (así flatpickr abre ahí sin preseleccionar)
    const _defaultFechaVisita = new Date(_anioPeriodo, _mesPeriodo, 1);

    // Inicializar flatpickr en los controles de rango de cada pestaña especial
    _fpInit('hm-inicio');
    _fpInit('hm-fin');
    _fpInit('angeles-inicio');
    _fpInit('angeles-fin');
    // Fecha de visita manual: abrir en el mes del periodo, sin fecha preseleccionada
    const _optsVisita = { defaultDate: _defaultFechaVisita };
    _fpInit('add-fecha-universo', _optsVisita);
    _fpInit('add-fecha-hm',       _optsVisita);
    _fpInit('add-fecha-angeles',  _optsVisita);
}

/** Inicializa flatpickr en un input del DOM (readonly para que solo use el picker) */
function _fpInit(inputId, opts) {
...
/** Devuelve la fecha guardada en el input del DOM previo (si existe) */
function _leerFechaCorteGuardada(inputId) {
    const el = document.getElementById(inputId);
...
/**
 * Renderiza la barra de controles por pestaña.
 * Universo: fecha visita + selector planta searchable + selector línea
 * Especiales: flatpickr rango inicio/fin + fecha visita + selector planta searchable (línea fija)
 */
function _renderControlesTab(seccion, fechaInicio, fechaFin, inicioVal, finVal) {
    const esEspecial = LIQ_CONFIG.LINEAS_ESPECIALES.includes(seccion);
    const seccionId  = seccion === 'HACEMOS MODA' ? 'hm' : seccion === 'ANGELES' ? 'angeles' : 'universo';

    // Para universo: obtener todas las líneas únicas de liqPlantas + reportes cargados,
    // excluyendo las especiales. Se construye dinámicamente.
    function _opcionesLineaUniverso() {
        const lineasSet = new Set(LIQ_CONFIG.LINEAS_UNIVERSO); // base mínima
        // Agregar todas las líneas presentes en liqPlantas
        (liqPlantas || []).forEach(p => {
            const l = (p.LINEA || p.linea || '').toUpperCase().trim();
            if (l && !LIQ_CONFIG.LINEAS_ESPECIALES.includes(l)) lineasSet.add(l);
        });
        // Agregar líneas de reportes ya cargados
        [...(liqReportes || []), ...(liqAprobaciones || [])].forEach(r => {
            const l = (r.LINEA || r.linea || '').toUpperCase().trim();
            if (l && !LIQ_CONFIG.LINEAS_ESPECIALES.includes(l)) lineasSet.add(l);
        });
        const sorted = [...lineasSet].sort();
        return sorted.map(l => `<option value="${l}">${l}</option>`).join('') +
               `<option value="__otra__">✏️ Otra línea...</option>`;
    }

    const inputStyle = 'padding:0.4rem 0.65rem;border:1px solid #e2e8f0;border-radius:6px;font-size:0.85rem;width:140px;background:white;cursor:pointer;';
    const labelStyle = 'display:flex;flex-direction:column;gap:3px;font-size:0.75rem;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:0.4px;';

    const rangoHtml = esEspecial ? `
        <label style="${labelStyle}">
            Inicio periodo
            <input type="text" id="${seccionId}-inicio" value="${inicioVal || _isoFecha(fechaInicio)}"
                readonly placeholder="Selecciona fecha" style="${inputStyle}">
        </label>
        <label style="${labelStyle}">
            Fin periodo
            <input type="text" id="${seccionId}-fin" value="${finVal || _isoFecha(fechaFin)}"
                readonly placeholder="Selecciona fecha" style="${inputStyle}">
        </label>
        ${seccion === 'HACEMOS MODA' ? `
        <label style="${labelStyle}">
            Días a descontar
            <input type="number" id="dias-desc-hm" min="0" max="15" value="0"
                style="padding:0.4rem 0.65rem;border:1px solid #e2e8f0;border-radius:6px;font-size:0.85rem;width:80px;background:white;"
                title="Días a descontar del valor quincenal de Hacemos Moda">
        </label>` : ''}
        <button onclick="reaplicarRangoEspecial('${seccion}')"
            style="background:var(--color-primary);color:white;border:none;padding:0.45rem 1rem;border-radius:7px;font-weight:600;font-size:0.82rem;cursor:pointer;align-self:flex-end;display:flex;align-items:center;gap:5px;">
            <i class="fas fa-sync-alt"></i> Aplicar rango
        </button>
        <div style="width:1px;height:32px;background:#e2e8f0;margin:0 4px;align-self:flex-end;"></div>
    ` : '';

    return `
        <div class="liq-tab-controls no-print"
            style="display:flex;flex-wrap:wrap;gap:0.65rem;align-items:flex-end;padding:0.75rem 1rem;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;margin-bottom:1.25rem;">
            ${rangoHtml}
            <label style="${labelStyle}">
                Fecha visita
                <input type="text" id="add-fecha-${seccionId}"
                    readonly placeholder="Selecciona fecha" style="${inputStyle}">
            </label>
            <label style="${labelStyle}flex:1;min-width:220px;">
                Planta / Productor
                <div style="position:relative;">
                    <input type="text" id="add-planta-${seccionId}"
                        placeholder="Escribir para filtrar..."
                        autocomplete="off"
                        oninput="_filtrarPlantasDropdown('${seccionId}','${esEspecial ? seccion : ''}')"
                        onfocus="_filtrarPlantasDropdown('${seccionId}','${esEspecial ? seccion : ''}')"
                        onblur="_cerrarDropdownPlanta('${seccionId}')"
                        style="padding:0.4rem 0.65rem;border:1px solid #e2e8f0;border-radius:6px;font-size:0.85rem;background:white;width:100%;">
                    <div id="dp-${seccionId}"
                        style="display:none;position:absolute;top:100%;left:0;right:0;max-height:220px;overflow-y:auto;background:white;border:1px solid #cbd5e1;border-top:none;border-radius:0 0 8px 8px;box-shadow:0 6px 16px rgba(0,0,0,0.12);z-index:500;">
                    </div>
                </div>
            </label>
            ${esEspecial ? '' : `
            <label style="${labelStyle}">
                Línea
                <select id="add-linea-${seccionId}"
                    onchange="_onCambioLinea('${seccionId}')"
                    style="padding:0.4rem 0.65rem;border:1px solid #e2e8f0;border-radius:6px;font-size:0.85rem;background:white;">
                    ${_opcionesLineaUniverso()}
                </select>
                <input type="text" id="add-linea-libre-${seccionId}"
                    placeholder="Escribe la línea..."
                    style="display:none;margin-top:4px;padding:0.4rem 0.65rem;border:1px solid #f59e0b;border-radius:6px;font-size:0.85rem;background:white;width:100%;">
            </label>`}
            <button onclick="agregarVisitaManual('${seccion}', '${seccionId}')"
                style="background:#10b981;color:white;border:none;padding:0.45rem 1rem;border-radius:7px;font-weight:600;font-size:0.82rem;cursor:pointer;align-self:flex-end;display:flex;align-items:center;gap:5px;">
...
/**
 * Filtra el dropdown de plantas según texto escrito y línea activa.
 * Usa liqPlantas (Supabase) + plantas ya en reportes cargados.
 */
window._filtrarPlantasDropdown = function(seccionId, seccionFija) {
    const input = document.getElementById(`add-planta-${seccionId}`);
    const dp    = document.getElementById(`dp-${seccionId}`);
    if (!input || !dp) return;

    const query = input.value.trim().toUpperCase();

    // Sin filtro por línea — mostrar todas las plantas siempre
    const lineasPermitidas = null;

    // Plantas de Supabase
    const deSupabase = (liqPlantas || []).map(p => ({
        nombre: (p.PLANTA || p.planta || '').toUpperCase(),
        linea:  (p.LINEA  || p.linea  || '').toUpperCase()
    })).filter(p => p.nombre);

    // Plantas de reportes ya cargados (por si no están en liqPlantas)
    const deReportes = [...(liqReportes||[]), ...(liqAprobaciones||[])].map(r => ({
        nombre: (r.PLANTA || r.planta || '').toUpperCase(),
        linea:  (r.LINEA  || r.linea  || '').toUpperCase()
    })).filter(p => p.nombre);

    // Unir y deduplicar por nombre
    const mapa = new Map();
    [...deSupabase, ...deReportes].forEach(p => {
        if (!mapa.has(p.nombre)) mapa.set(p.nombre, p);
    });
    let lista = [...mapa.values()];

    // Filtrar por texto
    if (query) lista = lista.filter(p => p.nombre.includes(query));

    lista.sort((a, b) => a.nombre.localeCompare(b.nombre));

    if (lista.length === 0) {
        dp.innerHTML = '<div style="padding:0.6rem 0.75rem;font-size:0.82rem;color:#94a3b8;">Sin resultados</div>';
        dp.style.display = 'block';
        return;
    }

    dp.innerHTML = lista.map(p => `
        <div onmousedown="_seleccionarPlanta('${seccionId}', '${p.nombre.replace(/\\/g,'\\\\').replace(/'/g,"\\'")}')"
            style="padding:0.45rem 0.75rem;cursor:pointer;font-size:0.85rem;border-bottom:1px solid #f1f5f9;display:flex;justify-content:space-between;align-items:center;gap:8px;"
            onmouseover="this.style.background='#eff6ff'" onmouseout="this.style.background='white'">
            <span style="font-weight:500;">${p.nombre}</span>
            <span style="font-size:0.7rem;color:#94a3b8;white-space:nowrap;">${p.linea}</span>
        </div>
    `).join('');
    dp.style.display = 'block';
};

/** Muestra/oculta el input de línea libre según la opción elegida */
window._onCambioLinea = function(seccionId) {
    const sel   = document.getElementById(`add-linea-${seccionId}`);
    const libre = document.getElementById(`add-linea-libre-${seccionId}`);
    if (!sel || !libre) return;
...
/**
 * Convierte la celda de nombre en un input inline para renombrar.
 * El nombre original se conserva como clave; el alias se guarda en localStorage.
 */
window.iniciarRenombrarPlanta = function(celda, nombreOriginal) {
    if (celda.querySelector('input')) return; // ya está editando
    const actual = _nombrePlanta(nombreOriginal);
    celda.innerHTML = `
        <input type="text" value="${actual.replace(/"/g,'&quot;')}"
            id="rename-input-${CSS.escape(nombreOriginal)}"
            style="width:100%;padding:2px 6px;border:1px solid #3f51b5;border-radius:4px;font-size:0.85rem;font-family:inherit;background:white;"
            onblur="confirmarRenombrar(this,'${nombreOriginal.replace(/'/g,"\\'")}','${CSS.escape(nombreOriginal)}')"
            onkeydown="if(event.key==='Enter'){this.blur();}if(event.key==='Escape'){cancelarRenombrar(this,'${nombreOriginal.replace(/'/g,"\\'")}');}">
    `;
    celda.querySelector('input').select();
};

window.confirmarRenombrar = function(input, nombreOriginal, escapedKey) {
    const nuevoNombre = input.value.trim().toUpperCase();
    if (nuevoNombre && nuevoNombre !== nombreOriginal) {
        _aliasPlanta[nombreOriginal] = nuevoNombre;
    } else if (!nuevoNombre || nuevoNombre === nombreOriginal) {
        delete _aliasPlanta[nombreOriginal]; // quitar alias si vuelve al original
    }
    _guardarAliasLS();
    // Actualizar la celda sin regenerar toda la liquidación
    input.parentElement.innerHTML = `
        <span ondblclick="iniciarRenombrarPlanta(this.parentElement,'${nombreOriginal.replace(/'/g,"\\'")}')">
            ${_nombrePlanta(nombreOriginal)}
        </span>
        ${nombreOriginal !== _nombrePlanta(nombreOriginal)
            ? `<span title="Nombre original: ${nombreOriginal}" style="font-size:0.65rem;color:#94a3b8;margin-left:4px;cursor:help;">✎</span>`
            : ''}
    `;
};

window.cancelarRenombrar = function(input, nombreOriginal) {
    input.parentElement.innerHTML = `
        <span ondblclick="iniciarRenombrarPlanta(this.parentElement,'${nombreOriginal.replace(/'/g,"\\'")}')">
            ${_nombrePlanta(nombreOriginal)}
        </span>
...
/**
 * Elimina un registro manual por su _id único y regenera.
 */
window.eliminarManualPorId = async function(id, tabActivo) {
    const idx = liqReportes.findIndex(r => r._manual && r._id === id);
    if (idx === -1) return;
    liqReportes.splice(idx, 1);
    _guardarManualesLS();
    await generarLiquidacion();
    if (tabActivo) cambiarPestana(tabActivo);
};

window._seleccionarPlanta = function(seccionId, nombre) {
    const input = document.getElementById(`add-planta-${seccionId}`);
    const dp    = document.getElementById(`dp-${seccionId}`);
...
window._cerrarDropdownPlanta = function(seccionId) {
    // Pequeño delay para que onmousedown del item se dispare antes del blur
    setTimeout(() => {
        const dp = document.getElementById(`dp-${seccionId}`);
        if (dp) dp.style.display = 'none';
    }, 160);
...
function renderizarGrupoTab(liquidacion, seccion, fechaInicio, fechaFin, fechasDescontadas = []) {
    ...
    lineasFiltradas.forEach(linea => {
        ...
        Object.values(lineaData.plantas).forEach(planta => {
            const nombreMostrado = _nombrePlanta(planta.nombre);
            const tieneAlias = nombreMostrado !== planta.nombre;
            const nombreEsc = planta.nombre.replace(/'/g,"\\'");
            html += `
                <tr>
                    <td class="planta-name no-print-edit" style="cursor:default;">
                        <span ondblclick="iniciarRenombrarPlanta(this.parentElement,'${nombreEsc}')"
                            title="Doble clic para renombrar"
                            style="cursor:text;">${nombreMostrado}</span>
                        ${tieneAlias ? `<span title="Nombre original en Supabase: ${planta.nombre}" style="font-size:0.65rem;color:#94a3b8;margin-left:4px;cursor:help;">✎</span>` : ''}
                    </td>
                    <td>${linea}</td>
                    ${diasPeriodo.map(dia => {
        ...
        html += `
            <tr class="total-row" style="background: #f8fafc; font-weight: 700;">
                <td class="planta-name" colspan="2">TOTAL</td>
                ${diasPeriodo.map(dia => {
        ...
        // Chips de registros manuales de esta línea — visibles solo en pantalla, no en PDF
        const tabActivoId = seccion === 'HACEMOS MODA' ? 'hacemos-moda'
                          : seccion === 'ANGELES'       ? 'angeles'
                          : 'universo';
        const manualesLinea = liqReportes.filter(r =>
            r._manual && (r.LINEA || r.linea || '').toUpperCase() === linea
        );
        if (manualesLinea.length > 0) {
            html += `<div class="no-print" style="margin:-0.5rem 0 1.25rem;display:flex;flex-wrap:wrap;gap:6px;align-items:center;">
                <span style="font-size:0.72rem;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:0.4px;">Manuales:</span>`;
            manualesLinea.forEach(r => {
                const fDisp = (r.FECHA||r.fecha||'').split('/').length === 3
                    ? `${(r.FECHA||r.fecha).split('/')[0]}/${(r.FECHA||r.fecha).split('/')[1]}`
                    : (r.FECHA||r.fecha||'');
                html += `<span style="display:inline-flex;align-items:center;gap:4px;background:#e0e7ff;color:#3730a3;border-radius:999px;padding:2px 8px;font-size:0.75rem;font-weight:600;">
                    ${r.PLANTA||r.planta} — ${fDisp}
                    <button onmousedown="eliminarManualPorId('${r._id}','${tabActivoId}')"
                        title="Eliminar este registro manual"
                        style="background:none;border:none;cursor:pointer;color:#6366f1;font-size:0.9rem;line-height:1;padding:0 0 0 2px;display:flex;align-items:center;">&times;</button>
                </span>`;
            });
            html += `</div>`;
        }
...
window.cambiarPestana = function (pestanaId) {
    ...
    // Inicializar flatpickr de los inputs de la pestaña que acaba de mostrarse
    // (necesario porque estaban en display:none al momento del primer init)
    const seccionId = pestanaId === 'hacemos-moda' ? 'hm'
                    : pestanaId === 'angeles'       ? 'angeles'
                    : 'universo';
    const _mes  = parseInt(document.getElementById('mesSelector')?.value ?? new Date().getMonth());
    const _anio = parseInt(document.getElementById('anioSelector')?.value ?? new Date().getFullYear());
    const _optsVisita = { defaultDate: new Date(_anio, _mes, 1) };
    _fpInit(`${seccionId}-inicio`);
    _fpInit(`${seccionId}-fin`);
    _fpInit(`add-fecha-${seccionId}`, _optsVisita);
};
...
async function imprimirLiquidacion() {
    ...
    tabsContent.forEach((tab, index) => {
        ...
        // Remover elementos no deseados (controles, botones, navegación)
        tabClone.querySelectorAll('button, .tabs-navigation, .liq-tab-controls, .no-print').forEach(el => el.remove());
...
/**
 * Reaplicar rango de una línea especial: re-genera la liquidación
 * leyendo las nuevas fechas de corte del tab activo.
 */
window.reaplicarRangoEspecial = async function(seccion) {
    await generarLiquidacion();
    // Mantener pestaña activa
    const tabId = seccion === 'HACEMOS MODA' ? 'hacemos-moda' : 'angeles';
    cambiarPestana(tabId);
};

/**
 * Agrega una visita manual al liqReportes en memoria, persiste en localStorage, y re-genera.
 */

function _guardarManualesLS() {
    const manuales = liqReportes.filter(r => r._manual);
    try { localStorage.setItem(_MANUAL_LS_KEY, JSON.stringify(manuales)); } catch(e) {}
}

function _cargarManualesLS() {
    try {
        const raw = localStorage.getItem(_MANUAL_LS_KEY);
        return raw ? JSON.parse(raw) : [];
    } catch(e) { return []; }
}

/** Genera un ID único para cada registro manual */
function _nuevoManualId() {
    return `m_${Date.now()}_${Math.random().toString(36).slice(2,7)}`;
}

window.agregarVisitaManual = async function(seccion, seccionId) {
    const fechaEl  = document.getElementById(`add-fecha-${seccionId}`);
    const plantaEl = document.getElementById(`add-planta-${seccionId}`);
    const lineaEl  = document.getElementById(`add-linea-${seccionId}`);

    const fecha  = fechaEl  ? fechaEl.value.trim()  : '';
    const planta = plantaEl ? plantaEl.value.trim().toUpperCase() : '';

    // Leer línea: si eligió "Otra..." usar el input libre; si es pestaña especial, usar seccion
    let linea;
    if (lineaEl) {
        if (lineaEl.value === '__otra__') {
            const libreEl = document.getElementById(`add-linea-libre-${seccionId}`);
            linea = libreEl ? libreEl.value.trim().toUpperCase() : '';
        } else {
            linea = lineaEl.value;
        }
    } else {
        linea = seccion; // especiales: fija
    }

    if (!fecha || !planta) {
        alert('Completa la fecha y el nombre de la planta.');
        return;
    }
    if (!linea) {
        alert('Escribe el nombre de la línea.');
        return;
    }
    ...
    const registroManual = { _id: _nuevoManualId(), FECHA: fechaNorm, PLANTA: planta, LINEA: linea, _manual: true };
    liqReportes.push(registroManual);

    // Si la línea es nueva (no especial y no estaba en LINEAS_UNIVERSO), agregarla para que aparezca en tabla
    if (!LIQ_CONFIG.LINEAS_ESPECIALES.includes(linea) && !LIQ_CONFIG.LINEAS_UNIVERSO.includes(linea)) {
        LIQ_CONFIG.LINEAS_UNIVERSO.push(linea);
    }

    // Persistir en localStorage
    _guardarManualesLS();

    if (plantaEl) plantaEl.value = '';

    const tabActivo = seccion === 'HACEMOS MODA' ? 'hacemos-moda'
                    : seccion === 'ANGELES'       ? 'angeles'
                    : 'universo';
    await generarLiquidacion();
    cambiarPestana(tabActivo);
};

/**
 * Elimina solo los registros manuales de liqReportes y de localStorage.
 * Los datos de Supabase se conservan intactos.
 */
window.limpiarManuales = async function() {
    const cuantos = liqReportes.filter(r => r._manual).length;
    if (cuantos === 0) { alert('No hay registros manuales para limpiar.'); return; }
    if (!confirm(`¿Eliminar los ${cuantos} registro(s) agregado(s) manualmente? Los datos de Supabase se conservan.`)) return;
    liqReportes = liqReportes.filter(r => !r._manual);
    try { localStorage.removeItem(_MANUAL_LS_KEY); } catch(e) {}
    await generarLiquidacion();
};

/**
 * Exporta la liquidación actual a Excel con 3 hojas: Universo, Ángeles, Hacemos Moda.
 * Usa SheetJS (xlsx) cargado dinámicamente si no está disponible.
 */
window.exportarExcelLiquidacion = async function() {
    const container = document.getElementById('liquidacionContainer');
    if (!container || container.innerHTML.includes('Selecciona el periodo')) {
        alert('Primero genera una liquidación.'); return;
    }
    ...
    const anio     = parseInt(document.getElementById('anioSelector').value);
    const quincena = parseInt(document.getElementById('quincenaSelector').value);
    const periodoInicio = quincena === 1 ? new Date(anio, mes, 1)  : new Date(anio, mes, 16);
    const periodoFin    = quincena === 1 ? new Date(anio, mes, 15) : new Date(anio, mes + 1, 0);

    const corteAngeles     = { inicio: _parseIso(_leerFechaCorteGuardada('angeles-inicio')) || periodoInicio, fin: _parseIso(_leerFechaCorteGuardada('angeles-fin')) || periodoFin };
    const corteHacemosModa = { inicio: _parseIso(_leerFechaCorteGuardada('hm-inicio'))      || periodoInicio, fin: _parseIso(_leerFechaCorteGuardada('hm-fin'))      || periodoFin };

    const diasDescontarStr = document.getElementById('descuentosInput')?.value || '';
    const fechasDescontadas = diasDescontarStr ? diasDescontarStr.split(',').map(d => d.trim()).filter(Boolean) : [];
    const diasDescontados = fechasDescontadas.length;

    const reportesFiltrados = filtrarReportesPorPeriodoConCorte(liqReportes, periodoInicio, periodoFin, corteAngeles, corteHacemosModa);
    const aprobacionesFiltradas = filtrarReportesPorPeriodoConCorte(liqAprobaciones, periodoInicio, periodoFin, corteAngeles, corteHacemosModa);
    const todos = [...reportesFiltrados, ...aprobacionesFiltradas];
    const liquidacion = procesarLiquidacion(todos, periodoInicio, periodoFin, diasDescontados);

    const wb = XLSX.utils.book_new();

    // Generar hoja por sección
    const secciones = [
        { nombre: 'UNIVERSO',     tabId: 'universo',      inicio: periodoInicio,        fin: periodoFin },
        { nombre: 'ANGELES',      tabId: 'ANGELES',       inicio: corteAngeles.inicio,  fin: corteAngeles.fin },
        { nombre: 'HACEMOS MODA', tabId: 'HACEMOS MODA',  inicio: corteHacemosModa.inicio, fin: corteHacemosModa.fin }
    ];

    secciones.forEach(({ nombre, tabId, inicio, fin }) => {
        const lineasSeccion = Object.keys(liquidacion.lineas).filter(l =>
            tabId === 'universo' ? !LIQ_CONFIG.LINEAS_ESPECIALES.includes(l) : l === tabId
        );

        if (lineasSeccion.length === 0) {
            // Hoja vacía con encabezado
            const ws = XLSX.utils.aoa_to_sheet([['Sin datos para esta sección']]);
            XLSX.utils.book_append_sheet(wb, ws, nombre);
            return;
        }

        const diasPeriodo = obtenerDiasPeriodo(inicio, fin);
        const rows = [];

        lineasSeccion.forEach(linea => {
            const ld = liquidacion.lineas[linea];

            // Encabezado de sección
            rows.push([`LÍNEA: ${linea}`]);
            // Header columnas
            rows.push(['PLANTA', 'LÍNEA', ...diasPeriodo.map(d => `Día ${d}`), '# VISITAS', 'VALOR']);

            Object.values(ld.plantas).forEach(p => {
                rows.push([
                    _nombrePlanta(p.nombre),
                    linea,
                    ...diasPeriodo.map(d => p.visitasPorDia[d] || ''),
                    p.totalVisitas,
                    p.valor
                ]);
            });
            ...
            rows.push([
                'TOTAL', '',
                ...diasPeriodo.map(d => Object.values(ld.plantas).reduce((s, p) => s + (p.visitasPorDia[d] || 0), 0) || ''),
                ld.totales.visitas,
                ld.totales.valor
            ]);
            rows.push([]); // espacio entre líneas
        });

        // Resumen
        const totalSeccion = Object.keys(liquidacion.lineas)
            .filter(l => tabId === 'universo' ? !LIQ_CONFIG.LINEAS_ESPECIALES.includes(l) : l === tabId)
            .reduce((s, l) => s + liquidacion.lineas[l].totales.valor, 0);
        rows.push([]);
        rows.push(['TOTAL A PAGAR', '', ...Array(diasPeriodo.length).fill(''), '', totalSeccion]);

        const ws = XLSX.utils.aoa_to_sheet(rows);

        // Formato moneda en columna VALOR y TOTAL A PAGAR
        const range = XLSX.utils.decode_range(ws['!ref']);
        const colValor = diasPeriodo.length + 3; // PLANTA + LINEA + dias + #VISITAS + VALOR
        for (let R = range.s.r; R <= range.e.r; R++) {
            const cell = ws[XLSX.utils.encode_cell({ r: R, c: colValor })];
    ...
    const auditora = obtenerAuditora([...liqReportes, ...liqAprobaciones]);
    const nombreArchivo = `Liquidacion_${auditora.replace(/\s+/g,'_')}_Q${quincena}_${String(mes+1).padStart(2,'0')}_${anio}.xlsx`;
